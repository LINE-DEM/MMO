...
class StatusService : Singleton<StatusService>,IDisposable {
	...
	//一个状态类型（例如money item） 对应一个委托（委托里注册许多事件）
	...
	//状态接收入口 传入状态列表（许多Status（id value Type Action））
	...
	{
		Debug.LogFormat("状态信息： [{0}] [{1}] {2} {3}", status.Type, status.Action, status.Id, status.Value);
