...
namespace Services
{
    class UserService : Singleton<UserService>, IDisposable
    {
        ...
        {
            Debug.LogFormat("OnUserLogin:Result:{0} Errormsg:[{1}]", response.Result, response.Errormsg);
        ...
        public void SendRegister(string user, string psw)
        {
            ...
            {
                this.pendingMessage = message;//一个队列
                this.ConnectToServer();
            }
        }
        ...
        {
            Debug.LogFormat("UserCreatCharacterRequest::角色名称 :{0} 角色属性:{1}", name, characterClass);
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
        ...
        void OnCharacterEnter(object sender , MapCharacterEnterResponse respone)
        {
            Debug.LogFormat("OnMapCharacterEnter:");
            NCharacterInfo info = respone.Characters[0];
           // User.Instance.CurrentCharacter = info;
            SceneManager.Instance.LoadScene(DataManager.Instance.Maps[respone.mapId].Resource);
        }
    }
}
