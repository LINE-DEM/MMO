...
namespace Network
{
    public class MessageDispatch<T> : Singleton<MessageDispatch<T>>
    {
        public void Dispatch(T sender, SkillBridge.Message.NetMessageResponse message)
        {
            ...
            if(message.skillAddReq != null) { MessageDistributer<T>.Instance.RaiseEvent(sender, message.skillAddReq); }
            if(message.skillAddRes != null) { MessageDistributer<T>.Instance.RaiseEvent(sender, message.skillAddRes); }
